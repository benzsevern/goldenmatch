# GoldenMatch Wiki

Welcome to the GoldenMatch wiki. GoldenMatch is an entity resolution toolkit for record deduplication, list matching, and golden record creation.

## Contents

### Getting Started
- [Installation](Installation.md)
- [Quick Start Guide](Quick-Start.md)
- [Zero-Config Mode](Zero-Config.md)

### Core Concepts
- [Pipeline Overview](Pipeline-Overview.md)
- [Matchkeys & Scoring](Matchkeys-and-Scoring.md)
- [Blocking Strategies](Blocking-Strategies.md)
- [Golden Records](Golden-Records.md)

### Advanced Features
- [Interactive TUI](Interactive-TUI.md)
- [GPU Routing & Vertex AI](GPU-Routing.md)
- [Database Integration](Database-Integration.md)
- [LLM Boost](LLM-Boost.md)
- [Embedding & Semantic Matching](Embeddings.md)
- [Settings & Persistence](Settings.md)

### Reference
- [Config File Reference](Config-Reference.md)
- [CLI Reference](CLI-Reference.md)
- [Transforms Reference](Transforms.md)
- [Architecture](Architecture.md)
- [Benchmarks](Benchmarks.md)
